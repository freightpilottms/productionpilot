...
export default function DobavljacKartica({ params }) {
  ...
  async function loadPromet() {
    ...
    try {
      const r = await fetch(`/api/karticaDobavljac?${qs.toString()}`, {
        cache: "no-store",
      });
  ...
  useEffect(() => {
    ...
    async function loadHeader() {
      ...
      try {
        const url = `/api/dobavljaci?q=${encodeURIComponent(
          subjekt
  ...
  function buildKarticaReport() {
    return {
      title: "Kartica dobavljača",
      subtitle: subjekt,
      subject: `Kartica dobavljača - ${subjekt}`,
      meta: [
        ["Dobavljač", row?.Subjekt || subjekt],
        ["Šifra", row?.Sifra || "—"],
        ["Period", periodLabel()],
      ],
  ...
  return (
    <main className="container page">
      <DesktopAppHeader title="Kartica dobavljača" subtitle={subjekt} status={loading ? "Učitavanje…" : mode} />

      <div className="topbar mobileOnlyHeader">
        <div>
          <div className="brand">Kartica dobavljača</div>
          <div
            className="subtitle"
            title={subjekt}
            style={{
      ...
      >
        <div className="card clickable" role="button" tabIndex={0}>
          <div className="cardTitle">Dobavljač</div>
          <div
            className="big"
      ...
      {/* Desktop filter stays at the top */}
      ...
      {/* Mobile filter goes below table / empty / loading */}
